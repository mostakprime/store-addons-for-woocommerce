...
const RepeatableField = ({ name, options, savedData, handleChange }) => {
  ...
  // Initialize sections based on savedData using useEffect
  useEffect(() => {
    if (savedData && Array.isArray(savedData)) {
      if (savedData.length > 0) {
        setSections(savedData.map((data, index) => ({ id: index + 1, values: data })));
      } else {
        setSections([{ id: 1, values: {} }]);
      }
    } else {
      setSections([{ id: 1, values: {} }]);
    }
